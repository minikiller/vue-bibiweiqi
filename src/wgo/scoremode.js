(function(WGo) {
  ...
  ScoreMode.prototype.start = function() {
    ...
    this._click = function(x, y) {
      ...
      if (c == WGo.W) {
        if (this.position.get(x, y) == state.WHITE_STONE)
          territory_set(
            this.position,
            x,
            y,
            state.BLACK_CANDIDATE,
            state.BLACK_STONE
          );
        else {
          territory_reset(
        ...
        if (this.position.get(x, y) == state.BLACK_STONE)
          territory_set(
            this.position,
            x,
            y,
            state.WHITE_CANDIDATE,
            state.WHITE_STONE
          );
        else {
          territory_reset(
        ...
        if (p == state.BLACK_CANDIDATE)
          this.position.set(x, y, state.BLACK_NEUTRAL);
        else if (p == state.WHITE_CANDIDATE)
          this.position.set(x, y, state.WHITE_NEUTRAL);
        else if (p == state.BLACK_NEUTRAL)
          this.position.set(x, y, state.BLACK_CANDIDATE);
        else if (p == state.WHITE_NEUTRAL)
          this.position.set(x, y, state.WHITE_CANDIDATE);
      }
  ...
  ScoreMode.prototype.displayScore = function() {
    ...
    for (var i = 0; i < this.position.size; i++) {
      for (var j = 0; j < this.position.size; j++) {
        var s = this.position.get(i, j);
        var t = this.originalPosition.get(i, j);

        if (s == state.BLACK_CANDIDATE)
          score.black.push({ x: i, y: j, type: "mini", c: WGo.B });
        else if (s == state.WHITE_CANDIDATE)
          score.white.push({ x: i, y: j, type: "mini", c: WGo.W });
        else if (s == state.NEUTRAL) score.neutral.push({ x: i, y: j });

        if (t == WGo.W && s != state.WHITE_STONE)
          score.white_captured.push({ x: i, y: j, type: "outline", c: WGo.W });
        else if (t == WGo.B && s != state.BLACK_STONE)
          score.black_captured.push({ x: i, y: j, type: "outline", c: WGo.B });
      }
    }

    for (var i = 0; i < score.black_captured.length; i++) {
      this.board.removeObjectsAt(
        score.black_captured[i].x,
        score.black_captured[i].y
      );
    }

    for (var i = 0; i < score.white_captured.length; i++) {
      this.board.removeObjectsAt(
        score.white_captured[i].x,
        score.white_captured[i].y
      );
    }
    ...
    var sb =
      ...
      this.originalPosition.capCount.black;
    var sw =
      score.white.length +
    ...
    msg +=
      ...
      score.black.length +
      " + " +
      (score.white_captured.length + this.originalPosition.capCount.black) +
      " = " +
      sb +
      "</br>";
    msg +=
      ...
      score.white.length +
      " + " +
      (score.black_captured.length + this.originalPosition.capCount.white) +
      " + " +
      this.komi +
      " = " +
      sw +
    ...
    if (sb > sw)
      msg += "<p style='font-weight: bold;'>" + WGo.t("bwin", sb - sw) + "</p>";
    else
    ...
    while (change) {
      ...
      for (var i = 0; i < p.size; i++) {
        ...
        for (var j = 0; j < p.size; j++) {
          ...
          if (
            s == state.UNKNOWN ||
            s == state.BLACK_CANDIDATE ||
            s == state.WHITE_CANDIDATE
          ) {
            // get new state
            t = [
              p.get(j - 1, i),
            ...
            for (var k = 0; k < 4; k++) {
              if (t[k] == state.BLACK_STONE || t[k] == state.BLACK_CANDIDATE)
                b = true;
              else if (
                t[k] == state.WHITE_STONE ||
                t[k] == state.WHITE_CANDIDATE
              )
                w = true;
        ...
        click: function(player) {
          if (this.selected) {
            ...
            this._score_mode = new WGo.ScoreMode(
              player.kifuReader.game.position,
              player.board,
              player.kifu.info.KM || 0.5,
              player.notification
            );
