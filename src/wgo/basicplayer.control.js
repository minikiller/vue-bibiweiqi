(function(WGo, undefined) {
  ...
  var prepare_dom = function(player) {
    ...
    for (var w in Control.widgets) {
      widget = new Control.widgets[w].constructor(
        player,
        Control.widgets[w].args
      );
  ...
  var Control = WGo.extendClass(WGo.BasicPlayer.component.Component, function(
    player
  ) {
    this.super(player);
  ...
  control.Clickable.prototype.init = function(player, args) {
    ...
    // this.element.addEventListener("mouseover", fn1);
  ...
  var player_menu = function(player) {
    ...
    if (!player.menu) {
      ...
      for (var i in Control.menu) {
        widget = new Control.menu[i].constructor(
          player,
          Control.menu[i].args,
          true
        );
