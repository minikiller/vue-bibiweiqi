...
(function(WGo, undefined) {
  ...
  var sgf_write_node = function(node, output) {
    ...
    if (node.setup) {
      ...
      for (var i in node.setup) {
        if (node.setup[i].c == WGo.B)
          AB.push(sgf_coordinates(node.setup[i].x, node.setup[i].y));
        else if (node.setup[i].c == WGo.W)
          AW.push(sgf_coordinates(node.setup[i].x, node.setup[i].y));
    ...
    for (var key in props) {
      ...
      if (key == "turn")
        output.sgf += "PL[" + (props[key] == WGo.B ? "B" : "W") + "]";
      else if (key == "comment")
        output.sgf += "C[" + sgf_escape(props[key]) + "]";
  ...
  var concat_changes = function(ch_orig, ch_new) {
    ...
    ch_orig.remove = set_subtract(ch_orig.remove, ch_new.add).concat(
      ch_new.remove
    );
  };
  ...
  KifuReader.prototype = {
    ...
    next: function(i) {
      ...
      //sunlf add
      if (this.change.remove.length > 0) {
        document.getElementById("audioDead").play();
      } else {
